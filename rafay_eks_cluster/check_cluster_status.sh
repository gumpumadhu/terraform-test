...
sleep 60
CLUSTER_STATUS=`./rctl get cluster -p dev1 ${CLUSTER_NAME} -o json |jq '.status'|cut -d'"' -f2`
PROVISION_STATUS=`./rctl get cluster -p dev1 ${CLUSTER_NAME} -o json |jq '.provision.status' |cut -d'"' -f2`
...
do
  ...
  PROVISION_STATE=`./rctl get cluster -p dev1 ${CLUSTER_NAME} -o json | jq '.provision.running_state' |cut -d'"' -f2`
...
then
    ...
    CLUSTER_HEALTH=`./rctl get cluster -p dev1 ${CLUSTER_NAME} -o json | jq '.health' |cut -d'"' -f2`
